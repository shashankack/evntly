...
app.post('/clubs/:id/register', async (c) => {
	try {
		const clubSlug = c.req.param('id');
		if (!clubSlug || typeof clubSlug !== 'string') return c.json({ error: 'Invalid club slug' }, 400);
		...
		// Get club by slug
		const [club] = await db.select().from(clubs).where(eq(clubs.slug, String(clubSlug))).limit(1).execute();
		...
		if (existing.length > 0) {
			return c.json({ message: 'User is already a member of this club' }, 200);
		}
	...
	return c.json({ message: 'Club registration successful', membership }, 200);
	} catch (error) {
