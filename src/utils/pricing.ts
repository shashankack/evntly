...
export function parsePricingConfig(value: unknown): ActivityPricingConfig {
	...
	const config = nestedConfig && typeof nestedConfig === 'object' ? (nestedConfig as Record<string, unknown>) : rawConfig;
	const rawAddOns = config.addOns ?? config.addons ?? config.add_ons;

	return {
		baseLabel:
			typeof config.baseLabel === 'string' ? config.baseLabel : typeof config.base_label === 'string' ? config.base_label : undefined,
		baseSeatCount:
		...
		addOns: Array.isArray(rawAddOns)
			? rawAddOns
					.map((addOn) => {
						...
						const parsedAddOn: ActivityPricingAddOn = {
							id,
							label,
							pricePaise,
							...
							maxQuantity:
								item.maxQuantity !== undefined
									? Math.max(0, toInteger(item.maxQuantity, 1))
									: item.max_quantity !== undefined
										? Math.max(0, toInteger(item.max_quantity, 1))
										: undefined,
						};
						const seatCountValue =
							item.seatCount !== undefined
								? Math.max(0, toInteger(item.seatCount, 0))
								: item.seat_count !== undefined
									? Math.max(0, toInteger(item.seat_count, 0))
									: undefined;
						if (seatCountValue !== undefined) {
							parsedAddOn.seatCount = seatCountValue;
						}
						return parsedAddOn;
					})
...
export function computeRegistrationPricing(params: {
	...
	const selectedAddOns = selections
		.map((selection) => {
			...
			const seatCount = Math.max(0, definition.seatCount ?? 0);
